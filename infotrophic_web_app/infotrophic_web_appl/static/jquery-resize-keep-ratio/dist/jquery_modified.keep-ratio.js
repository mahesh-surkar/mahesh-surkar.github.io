    ...
    var resize = function($el, options, forceRendering) {
        ...
        if (options.calculate === 'auto') {
             ...
             resizeFunction = function() {
            	 ...
            	 //console.log( "heights " +$el.height() +""+ $el.parent().height());
            	 if ($el.height() > $el.parent().height())
            	 {
            		
            		 $el.height(Math.round($el.parent().height()));
            		 var height = $el.height();
            		 $el.width(Math.round(height * options.ratio));
            		 //console.log( "heights11 " +$el.height() +""+ $el.parent().height());
            	 }
             };
        }
